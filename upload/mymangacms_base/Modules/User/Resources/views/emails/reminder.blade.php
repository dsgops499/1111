...
<html>
    <head>
        ...
        <title>{{ trans('user::messages.front.auth.reset_password') }}</title>
        <style type="text/css">
    ...
    <body>
        <div class="main">
            ...
            <div class="content-block">
                {{ trans('user::messages.front.auth.reset_password_link') }}<br/>{{ URL::to("auth/reset/{$id}/{$code}") }}
            </div>
            <div class="content-block">
                <?php $authPrefix = env('AUTH_PREFIX', 'auth'); ?>
                <a href='{{ URL::to("$authPrefix/reset/{$id}/{$code}") }}' class="btn-primary">
                    {{ trans('user::messages.front.auth.reset_password') }}
                </a>
            </div>
            <div class="content-block">
