...
class SentinelAuthentication implements Authentication
{
    ...
    public function login(array $credentials, $remember = false)
    {
        try {
            ...
            return trans('user::messages.front.auth.invalid_login_password');
        } catch (NotActivatedException $e) {
            return trans('user::messages.front.auth.account_not_validated');
        } catch (ThrottlingException $e) {
            $delay = $e->getDelay();

            return trans('user::messages.front.auth.account_is_blocked', ['delay' => $delay]);
        }
    }
    ...
    /**
     * Determines if the current user has access to given permissions
     * @param $permissions
     * @return bool
     */
